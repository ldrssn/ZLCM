...
const ItemCard: React.FC<ItemCardProps> = ({ item, onEdit, onUpdate, activeColorFilter }) => {
  ...
  const shadowStyle = useMemo(() => {
    const itemColorHex = COLOR_MAP[item.color] || '#cccccc';
  ...
  const glowStyle = useMemo(() => {
    if (activeColorFilter === item.color) {
      const itemColorHex = COLOR_MAP[item.color] || '#cccccc';
      const rgb = hexToRgb(itemColorHex);
      if (rgb) {
  ...
  return (
    <div className="w-full max-w-[280px] h-[420px] [perspective:1000px] group mx-auto">
      ...
      >
        {/* FRONT FACE */}
        <div 
          className="absolute inset-0 w-full h-full [backface-visibility:hidden] rounded-xl overflow-hidden bg-white shadow-lg cursor-pointer"
          style={shadowStyle}
          onClick={handleFlip}
        >
            ...
            <div className="flex-grow overflow-y-auto pr-2 space-y-2 text-sm">
                ...
                <div className="flex justify-between items-center border-b border-zinc-600 pb-1">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="w-4 h-4 rounded-full border border-zinc-500" style={{ backgroundColor: COLOR_MAP[item.color] }}></span>
