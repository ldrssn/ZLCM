    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300">
      ...
      <div className="flex items-center p-4 cursor-pointer" onClick={handleToggleExpand}>
        ...
        <div className="flex-grow ml-4">
          <div className="flex justify-between items-start">
            <div>
              ...
              {item.isSold && <span className="text-xs font-bold text-red-500 bg-red-100 px-2 py-0.5 rounded-full">VERKAUFT</span>}
            </div>
            <div className="hidden sm:flex items-center space-x-1.5 text-sm bg-gray-100 text-gray-700 px-3 py-1 rounded-full">
        ...
        <div className="ml-4 flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 text-gray-400 transition-transform duration-300 ${isExpanded ? 'transform rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="space-y-2 text-sm text-gray-700">
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="w-4 h-4 rounded-full border border-gray-300" style={{ backgroundColor: COLOR_MAP[item.color] }}></span>
                ...
                {item.purchasePrice && <div className="flex justify-between"><span>Kaufpreis:</span> <span>€{item.purchasePrice.toFixed(2)}</span></div>}
                 {item.isSold && item.sellingPrice && (
                    <div className="flex justify-between text-green-600 font-semibold"><span>Verkaufspreis:</span> <span>€{item.sellingPrice.toFixed(2)}</span></div>
                )}
                ...
                {item.notes && (
                    <div className="pt-2">
                        <p className="font-semibold text-gray-800">Notizen:</p>
                        <p className="text-gray-600 bg-gray-100 p-2 rounded-md text-xs"><em>{item.notes}</em></p>
                    </div>
                )}
            </div>
            <div className="pt-4 mt-3 border-t border-gray-200 flex items-center justify-between gap-2">
                <button onClick={() => onEdit(item)} className="flex-grow px-3 py-2 bg-gray-200 text-gray-800 text-xs font-semibold rounded-md hover:bg-gray-300 transition-colors">Bearbeiten</button>
